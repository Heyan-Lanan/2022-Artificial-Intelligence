...
private:
    ...
    bool set_insert(const char *state);
    string h_func;
};
...
void IDAStar::graph_search() {
    ...
    int doorsill;          //TODO
    if (this->h_func == "h1")
        doorsill = this->state_manager->h_function(n->state);
    else
        doorsill = this->state_manager->h_function2(n->state);

    this->visited.emplace(n->state, INFO{true, n});
    while (true) {
        int temp = DFS(n, doorsill);
        if (temp == -1) {
            return;
        }
        if (temp == this->INFINITY) {
            return;
        }
        doorsill = temp;
    }
...
bool IDAStar::check(const char *b1, const char *b2) {
    for (int i = 0; i < DIM * DIM + 1; ++i) {
        if (b1[i] != b2[i]) {
            return false;
        }
...
AStar::AStar(const char *init_state, const char *dest_state, string h_func) {
    ...
    n->depth = 0;
    n->parent = nullptr;
    this->set_insert(n->state);
    this->nodes.emplace_back(n);
    this->state_manager = new state_manage(dest_state);
}

bool AStar::set_insert(const char *state) {
    std::string s(state);
    if (this->set_1.find(s) == this->set_1.end()) {
        this->set_1.insert(s);
        return true;
    }
    return false;
...
Node *AStar::graph_search() {
    ...
    while (!fringe.empty()) {  //队列不空则继续抽取最小节点
        auto t = fringe.top();
        fringe.pop();
        f = t.first.first;
        g = t.first.second;
        ...
        auto succeeds = state_manage::get_succeed_states(n->state); // 找后继节点
        for (auto &s: succeeds) {
            if (this->set_insert(s.second)) {   //对每个后继节点去重
                Node *n_temp = new Node;
                n_temp->state = s.second;
                n_temp->movement = s.first;
                n_temp->depth = n->depth + 1;
                ...
                fringe.push(std::make_pair(std::make_pair(n_temp_f, n_temp_g), n_temp));//插入队列中
                this->nodes.emplace_back(n_temp);
            }
        }
    }
...
bool AStar::check(const char *b1, const char *b2) {
